/**
 * episode10 字幕 - 基于实际配音时长
 * 
 * 生成日期：2026-03-05
 * 总时长：568.2秒 = 17045帧 (@30fps)
 * 
 * 场景时长（基于配音实际时长）:
 * - Scene 0: 48.7秒 = 1460帧 - Opening
 * - Scene 1: 68.6秒 = 2060帧 - Fractional Reserve
 * - Scene 2: 67.6秒 = 2027帧 - Debt Dollar
 * - Scene 3: 54.0秒 = 1618帧 - Derivatives
 * - Scene 4: 47.2秒 = 1418帧 - Fannie/Freddie
 * - Scene 5: 111.4秒 = 3342帧 - Gold Prison
 * - Scene 6: 73.5秒 = 2205帧 - Gold Counter
 * - Scene 7: 39.7秒 = 1191帧 - Rothschild
 * - Scene 8: 57.5秒 = 1724帧 - Summary
 */
import { SubtitleEntry } from "./index";

// 场景帧数配置（基于配音实际时长，30fps）
export const SCENE_FRAMES = [
  1460, // Scene 0: 48.7s - Opening
  2060, // Scene 1: 68.6s - Fractional Reserve
  2027, // Scene 2: 67.6s - Debt Dollar
  1618, // Scene 3: 54.0s - Derivatives
  1418, // Scene 4: 47.2s - Fannie/Freddie
  3342, // Scene 5: 111.4s - Gold Prison
  2205, // Scene 6: 73.5s - Gold Counter
  1191, // Scene 7: 39.7s - Rothschild
  1724, // Scene 8: 57.5s - Summary
];
...
// Scene 4: Fannie/Freddie (47.2s = 1418帧)
...
// Scene 5: Gold Prison (111.4s = 3342帧)
...
export const GoldCounterScene: SubtitleEntry[] = [
  ...
  { startFrame: 768, endFrame: 1322, text: "戴维·罗斯柴尔德解释道：我们在伦敦商品市场（包括黄金）交易的收入在过去5年中已经下降到不足我们业务总收入的1%，" },
...
export const episode10Subtitles: SubtitleEntry[] = [
  ...OpeningScene.map((s) => ({ ...s, startFrame: s.startFrame + SCENE_OFFSETS[0], endFrame: s.endFrame + SCENE_OFFSETS[0] })),
  ...FractionalReserveScene.map((s) => ({ ...s, startFrame: s.startFrame + SCENE_OFFSETS[1], endFrame: s.endFrame + SCENE_OFFSETS[1] })),
  ...DebtDollarScene.map((s) => ({ ...s, startFrame: s.startFrame + SCENE_OFFSETS[2], endFrame: s.endFrame + SCENE_OFFSETS[2] })),
  ...DerivativesScene.map((s) => ({ ...s, startFrame: s.startFrame + SCENE_OFFSETS[3], endFrame: s.endFrame + SCENE_OFFSETS[3] })),
  ...FannieFreddieScene.map((s) => ({ ...s, startFrame: s.startFrame + SCENE_OFFSETS[4], endFrame: s.endFrame + SCENE_OFFSETS[4] })),
  ...GoldPrisonScene.map((s) => ({ ...s, startFrame: s.startFrame + SCENE_OFFSETS[5], endFrame: s.endFrame + SCENE_OFFSETS[5] })),
  ...GoldCounterScene.map((s) => ({ ...s, startFrame: s.startFrame + SCENE_OFFSETS[6], endFrame: s.endFrame + SCENE_OFFSETS[6] })),
  ...RothschildScene.map((s) => ({ ...s, startFrame: s.startFrame + SCENE_OFFSETS[7], endFrame: s.endFrame + SCENE_OFFSETS[7] })),
  ...SummaryScene.map((s) => ({ ...s, startFrame: s.startFrame + SCENE_OFFSETS[8], endFrame: s.endFrame + SCENE_OFFSETS[8] })),
];
